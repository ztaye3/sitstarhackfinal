...
from .models import UserAccount, MerchantModel
from .serializers import UserAdminSerializer, UserCreateSerializer, UpdateUserProfileSerializer, MerchantSerializerRead, \
    MerchantSerializerWrite, UserAdminUpdateSerializer
...
class UserView(viewsets.ViewSet):
    ...
    def list(self, request, page=None, offset=None):
        ...
        try:
            ...
            serializer = UserAdminUpdateSerializer(queryset, many=True)
...
class UpdateUserProfile(ListAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = request.data
        is_merchant = request.data['is_merchant']
        ...
        if serializer.is_valid():
            ...
            merchant_data = {"identification": request.data['identification']}
            merchant_serializer = MerchantSerializerWrite(data=merchant_data)

            if merchant_serializer.is_valid() and is_merchant == "true":
                # Create a new merchant with only identification
                merchant_serializer.save()
                merchant_id = merchant_serializer.data.get('id')

                # Add user merchant
                merchant = MerchantModel.objects.get(id=merchant_id)
                merchant.merchant.add(user)
